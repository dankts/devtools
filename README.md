[![ci.yml](https://github.com/dankts/devtools/actions/workflows/ci.yml/badge.svg)](https://github.com/dankts/devtools/actions/workflows/ci.yml)
...
**Проблема:** в методе calculateProgress (строка 12), при возвращении объекта Integer может быть null
**Почему это важно:** приложение может упасть c NullPointerException
**Предложение:** сделать проверку через if или же обернуть в Optional<>

**Проблема:** название метода do(строка 32) обобщенное
**Почему это важно:** другим разработчиком в команде понадобится время чтобы разобраться
**Предложение:** переименовать 'do' в sendMessage

### Плохие комментарии

1. Код ужасен, ты сделал абсолютно неправильно на 32 строке, подумай как можно исправить
2. Ты где учился вообще? сделай как тебя учили и не задавай глупых вопросов

## Результаты само-ревью DVT-9

**Проблема:** нет Git Action Badge в README
**Почему это важно:** пользователи и ревьюверы не видят статус сборки
**Предложение:** добавить badge через markdown 

**Проблема:** в ci.yml (строка 17) указана не существующая версия
**Почему это важно:** action/checkout не сможет запуститься
**Предложение:** использоваться существующую версию, на данные момент v7

**Проблема:** в ci.yml (строка 30) чек стайл не затрагивает тесты
**Почему это важно:** checkstyle должен полностью проходить по всему проекту, чтобы отсутствовали ошибки стиля
**Предложение:** добавить в строку checkstyleTest